.data
arr1: .word  7, 91, 64, 3, 23, 1, 66, 12, 5, 128, 2, 6748           #array on which we are applying radix sort 
arr_size: .word 12
...
printarray:
    la $a0,array_str
    li $v0,4                 
    syscall                     # Print string array_str

    la $t3,arr_size
    lw $t2,($t3)                # Load length of array n into $t2
    addi $t0,$zero,0            # Counter to iterate over array
    la $s7,arr1                 # Load address of arr1 into $s7

samosa:
    lw $s6,($s7)
    beq $t0,$t2,jumpback        # Exit loop if counter t0 == n
    addi $t0,$t0,1              # Increment counter t0
    addi $s7,$s7,4              # Increment array address
    li $v0, 11                  # Print a comma
    li $a0, 44                  # ASCII value of comma
    syscall
    li $v0,1                    # Print integer value
    addi $a0,$s6,0              # Load integer into $a0
    syscall
    j samosa                    # Continue loop

jumpback:
    j printData                 # Jump to printData section

printData:
    la $a0,array_sorted_str
    li $v0,4                 
    syscall                      # Print string array_sorted_str
...
gajar:
    lw $s6,($s7)
    beq $t0,$t2,exit            # Exit loop if counter t0 == n
    addi $t0,$t0,1              # Increment counter t0
    addi $s7,$s7,4              # Increment array address
    li $v0, 11                  # Print a comma
    li $a0, 44                  # ASCII value of comma
...
evo:
    lw $t4,($s0)
    lw $t5,4($s0)
    beq $t1,$t3,hhh             # Jump to hhh if (j == n-1)
    addi $t1,$t1,1              # j++
...
hhh:
    addi $t0,$t0,1              # i++
    beq $t0,$t2,ggg             # Jump to ggg if (i == n)
    j ooo
...
ooo:
    li $t1,1                    # Reset j = 0
    la $s0,arr1                 # Reset array pointer
    j olx
